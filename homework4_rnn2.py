...
model.add(tf.keras.layers.GRU(50,dropout=0.5,recurrent_dropout=0.5))
model.add(tf.keras.layers.Dense(660, activation='elu'))
model.add(tf.keras.layers.GaussianDropout(0.3))
model.add(tf.keras.layers.Dense(400, activation='elu'))
model.add(tf.keras.layers.GaussianDropout(0.3))
...
model.save('first_try_rnn_a.model')

model.load_weights('first_try_rnn_a.model')
...
model = tf.keras.models.Sequential()
model.add(embedding_layer)
model.add(tf.keras.layers.GRU(32,dropout=0.5,recurrent_dropout=0.5))
...
model.save('first_try_rnn_b.model')

model.load_weights('first_try_rnn_b.model')
Y_preds = model.predict(X_test)
Y_preds_label = np.argmax(Y_preds, axis=1)
...
submit.to_csv("submitrnn"+now+".csv", index=False)
